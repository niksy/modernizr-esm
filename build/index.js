...
const config = async () => {
	const [mainEntry, source, features] = await Promise.all([
		...
		globby([
			`${modernizrDir}/src/**/*.js`,
			`!${modernizrDir}/src/{generate,slice,classes,${entryDependencies.join(
				','
			)}}.js`
		]),
		globby([
			`${modernizrDir}/feature-detects/${
				process.env.TEST
					? '{emoji,flash,img/webp,css/cssgrid,css/filters,audio/autoplay,indexeddb,audio,inputtypes,ambientlight}'
					: '**/*'
			}.js`
		])
	]);
